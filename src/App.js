      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "5px", flex: 1, justifyContent: "center" }}>
          {SHARED_DECK.map(card => {
            ...
            const cost = getCardCost(card);
            return (
              <div key={card.id} onClick={() => { if (canUse) onSelectCard(isSelected ? null : card); }} title={isUnlocked ? `${card.name} — ${card.description} (${cost}s)` : `Tier ${tierNum} locked`} style={{ position: "relative", width: "44px", height: "44px", borderRadius: "8px", overflow: "hidden", border: isSelected ? "2px solid #fff" : isUnlocked && !isUsed ? `2px solid ${card.color}66` : "2px solid rgba(255,255,255,0.06)", opacity: isUsed ? 0.3 : isUnlocked ? 1 : 0.25, cursor: canUse ? "pointer" : "default", boxShadow: isSelected ? `0 0 12px ${card.color}` : "none", flexShrink: 0 }}>
                {isUnlocked ? (
                  <>
                    <img src={card.image} alt={card.name} style={{ width: "100%", height: "100%", objectFit: "cover", filter: isUsed ? "grayscale(100%)" : "none" }} />
                    {!isUsed && <div style={{ position: "absolute", bottom: "2px", right: "2px", backgroundColor: "rgba(0,0,0,0.85)", color: "#ffd700", fontSize: "9px", fontWeight: "bold", padding: "1px 3px", borderRadius: "3px" }}>{cost}s</div>}
...
function PvpTabletLayout({ game, gameOver, gameOverDismissed, setGameOverDismissed, winner, theme, whiteTime, blackTime, formatTime, customStyles, onPieceDrop, onSquareClick, tier1Unlocked, tier2Unlocked, tier3Unlocked, selectedCard, setSelectedCard, usedCards, getCardCost, whiteCaptured, blackCaptured, getPieceSymbol, specialModeLabel, setChandraPlacementMode, setGuruMode, setShaniMode, setActivationMode, chandraPlacementMode, guruMode, shaniMode, activationMode, piecesInZones, resetGame, showChaosPopup, setShowChaosPopup, guruPickerMode, setGuruPickerMode, performResurrection }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minHeight: "100vh", width: "100%", backgroundColor: theme.background, color: theme.text, userSelect: "none", overscrollBehavior: "none" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "6px 0", width: "100%" }}>
        {specialModeLabel && (
          <div style={{ width: tabletBoard, backgroundColor: "#1a1a00", border: "1px solid #ffd700", borderRadius: "8px", padding: "7px 12px", marginBottom: "6px", fontSize: "13px", color: "#ffd700", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
            {specialModeLabel}
        ...
        {!selectedCard && !chandraPlacementMode && !guruMode && !shaniMode && piecesInZones.length > 0 && (
          <button onClick={() => setActivationMode(a => !a)} style={{ width: tabletBoard, marginBottom: "6px", padding: "10px", fontSize: "14px", backgroundColor: activationMode ? "#e94560" : "#ffd700", color: "#000", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "bold" }}>
            {activationMode ? "✕ Cancel Activation" : `⚡ Activate Power (${piecesInZones.length} available)`}
          </button>
        )}
        <div style={{ width: tabletBoard, textAlign: "center", fontSize: "12px", color: currentTurn === "w" ? "#4ecca3" : "#e94560", marginBottom: "4px" }}>
          {!gameOver && (currentTurn === "w" ? "⬜ White to move" : "⬛ Black to move")}
        </div>
        <div style={{ width: tabletBoard, height: tabletBoard, flexShrink: 0 }}>
